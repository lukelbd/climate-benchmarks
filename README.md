...
2. Iterate through the NetCDF files in `DIR`.
    ...
    * Pass the command-line call signature to the `benchmark` function for each script in the `testname` directory. For example: to run `python test.py file.nc`, we use `benchmark python test.py file.nc`.
